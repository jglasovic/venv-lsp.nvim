...
function M._init_lsp() -- for nvim v0.11
  for name, lsp in pairs(lsp_configs) do
    ...
    lsp_config.before_init = M._custom_before_init(lsp_config.before_init, M._update_config_wrapper(lsp.update_config))
  ...
  local on_setup = function(client_config)
    ...
    if lsp then
      client_config.on_new_config =
        lspconfig_pkg.util.add_hook_after(client_config.on_new_config, M._update_config_wrapper(lsp.update_config))
      client_config.on_attach = M._custom_on_attach(client_config.on_attach)
      client_config.root_dir =
        M._custom_lspconfig_root_dir(client_config.root_dir, lspconfig_pkg.util, lsp.default_config.root_markers)
      commands.init_auto_venv()
    end
  end
